-- Fix critical law firm data security vulnerabilities
...
-- 7. Create audit trigger for law firm access monitoring
CREATE OR REPLACE FUNCTION public.log_law_firm_access()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO 'public'
AS $$
BEGIN
  -- Log when someone accesses law firm data
  PERFORM public.log_sensitive_data_access(
    'law_firms',
    NEW.id,
    'law_firm_data_access'
  );
  RETURN NEW;
END;
$$;
...
CREATE TRIGGER log_law_firm_access_trigger
  AFTER SELECT ON public.law_firms
  FOR EACH ROW EXECUTE FUNCTION public.log_law_firm_access();