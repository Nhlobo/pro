-- Comprehensive admin access setup for boshomane@kutlwanoassociate.com - Fixed version
...
-- 5. Create storage policies for admin access (fixed syntax)
DROP POLICY IF EXISTS "Admin full access to all storage" ON storage.objects;
CREATE POLICY "Admin full access to all storage" ON storage.objects
FOR ALL USING (
  EXISTS (
    SELECT 1 FROM public.profiles 
    WHERE id = auth.uid() 
