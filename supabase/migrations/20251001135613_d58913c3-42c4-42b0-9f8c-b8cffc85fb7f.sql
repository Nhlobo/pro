-- Drop and recreate get_medical_experts_secure function with matter_types
DROP FUNCTION IF EXISTS public.get_medical_experts_secure();

CREATE FUNCTION public.get_medical_experts_secure()
 RETURNS TABLE(id uuid, first_name text, last_name text, expert_type text, province text, specializations text[], qualifications text, years_experience integer, status text, consultation_fees numeric, court_fees numeric, availability_notes text, created_at timestamp with time zone, updated_at timestamp with time zone, email_masked text, phone_masked text, address_masked text, pa_name_masked text, pa_phone_masked text, cv_document_url text, matter_types text[])
 LANGUAGE plpgsql
 SECURITY DEFINER
...
-- Drop and recreate get_medical_expert_display_safe function with matter_types
DROP FUNCTION IF EXISTS public.get_medical_expert_display_safe(uuid);

CREATE FUNCTION public.get_medical_expert_display_safe(expert_id uuid)
 RETURNS TABLE(id uuid, first_name text, last_name text, expert_type text, province text, specializations text[], qualifications text, years_experience integer, status text, consultation_fees numeric, court_fees numeric, availability_notes text, created_at timestamp with time zone, updated_at timestamp with time zone, email_masked text, phone_masked text, address_masked text, pa_name_masked text, pa_phone_masked text, cv_document_url text, matter_types text[])
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
