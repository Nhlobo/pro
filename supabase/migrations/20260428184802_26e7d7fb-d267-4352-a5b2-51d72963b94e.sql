...
AS $$
DECLARE
  v_payout_month INTEGER := EXTRACT(MONTH FROM (p_run_date + INTERVAL '1 month'))::INTEGER;
  v_payout_year INTEGER := EXTRACT(YEAR FROM (p_run_date + INTERVAL '1 month'))::INTEGER;
  v_period_start DATE := p_run_date;
  v_period_end DATE := (p_run_date + INTERVAL '1 month' - INTERVAL '1 day')::date;
  v_target INTEGER := 6;
