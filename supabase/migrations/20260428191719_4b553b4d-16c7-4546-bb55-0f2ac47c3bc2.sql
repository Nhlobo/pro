...
BEGIN
  ...
  WITH stats AS (
    ...
    SELECT
      ...
      CASE
        WHEN LOWER(COALESCE(p.position, '')) LIKE '%sales%consultant%' THEN 4
        ELSE 2
      END AS target_required,
    ...
    GROUP BY sc.id, sc.name, sc.user_id, p.email, au.email, p.position, st.total_appts
  ), to_issue AS (
    SELECT
      e.*,
