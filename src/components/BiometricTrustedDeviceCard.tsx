...
export const BiometricTrustedDeviceCard = () => {
  ...
  const enroll = async () => { if (!user?.email) return; setBusy(true); const result = await enrollTrustedDevice({ userId: user.id, userEmail: user.email, label: navigator.platform || 'Trusted device' }); if (!result.ok) toast.error(result.error || 'Could not enable biometric unlock.'); await load(); setBusy(false); };
