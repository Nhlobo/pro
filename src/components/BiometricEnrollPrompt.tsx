...
export const BiometricEnrollPrompt = () => {
  ...
  const enable = async () => { if (!user?.email) return; setBusy(true); const ok = await enrollTrustedDevice({ userId: user.id, userEmail: user.email, label: navigator.platform || 'Trusted device' }); setBusy(false); if (ok) setOpen(false); else dismiss(); };
