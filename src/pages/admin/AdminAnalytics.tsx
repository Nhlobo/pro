    ...
    <div className="space-y-4 md:space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 sm:gap-4">
        {[
          ...
          <Card key={kpi.label} className="rounded-none border-black/10 shadow-none min-w-0">
            <CardContent className="pt-4 pb-3 px-3 sm:px-4">
              <kpi.icon className="h-4 w-4 text-primary mb-2" />
              <p className="text-lg sm:text-xl md:text-2xl font-bold text-foreground truncate">{kpi.value}</p>
              <div className="flex flex-wrap items-center gap-1.5 sm:gap-2">
                <p className="text-[11px] text-muted-foreground truncate">{kpi.label}</p>
                <span className="text-[10px] text-success font-medium whitespace-nowrap">{kpi.trend}</span>
              </div>
            </CardContent>
          </Card>
        ))}
      ...
      <Card className="rounded-none border-black/10 shadow-none">
        ...
        <CardContent>
          <div className="w-full overflow-x-auto">
            <div className="flex items-end gap-2 sm:gap-4 h-48 min-w-[360px]">
              {monthlyData.map((m) => (
                <div key={m.month} className="flex-1 flex flex-col items-center gap-1 min-w-[36px]">
                  <div className="w-full flex gap-1 justify-center" style={{ height: '180px' }}>
                    <div className="flex flex-col justify-end w-4 sm:w-5">
                      <div
