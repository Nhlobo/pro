...
/**
 * Mirrors AttorneyMessages.tsx exactly — same underlying hooks, same
 * RLS boundary (external_portal_case_links), just wired to the Expert
 * Portal's not-linked state instead of the Attorney one. Note: unlike
 * most Attorney Portal pages, Expert Portal pages don't self-wrap in
 * their layout — <ExpertPortalRoute> in App.tsx does that — so this
 * file intentionally has no <ExpertPortalLayout> here either, matching
 * every sibling page (ExpertSchedule, ExpertCases, etc).
 */
const ExpertMessages: React.FC = () => {
  ...
  if (linkStatus === 'not_linked') return <ExpertNotLinkedState />;

  const selectedThread = threads?.find((t) => t.appointment_id === selectedId) || null;

  return (
    <PortalPage>
      <PortalHeader eyebrow="Expert Portal" title="Messages" description="Case-linked conversations with our team." icon={MessageSquare} />
