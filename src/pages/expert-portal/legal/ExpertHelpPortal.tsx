...
/**
 * Help & Support — Expert Portal, SIGNED-IN variant.
 *
 * Sibling of ExpertHelp.tsx, not a replacement for it. ExpertHelp is
 * reachable while signed OUT (footer link on ExternalPortalSignIn) as
 * well as signed in, so it can't assume a session exists and its "back"
 * link has to go to /external-portal/sign-in.
 *
 * This page is only ever reached from inside the authenticated portal —
 * currently the "Get Help" button on ExpertNotLinkedState, shown when a
 * signed-in expert has no linked profile and so no cases/history to
 * show. Because a session is guaranteed here, "back" goes to the actual
 * /expert-portal dashboard instead of bouncing a logged-in user back to
 * sign in. Routed behind ProtectedRoute in App.tsx for that reason.
 *
 * Content mirrors ExpertHelp.tsx — same support channels and common
 * issues — so keep the two in sync if either changes.
 */
const ExpertHelpPortal = () => (
  <LegalPageLayout
    title="Help & Support"
    description="Get help with the Medico-Legal Pro Expert Portal."
    backHref="/expert-portal"
    backLabel="Back to Dashboard"
  >
    <p>
      Need a hand with your Expert Portal? Our support team is available Monday to Friday,
      08:00 – 17:00 SAST.
    </p>
    ...
    <div className="not-prose grid gap-4">
      <LegalSectionCard number={1} title="Your profile isn't linked yet" icon={Link2Off}>
        <p>
          If your dashboard shows &ldquo;Expert Profile Not Linked&rdquo;, an administrator
          still needs to connect your account to your medical expert record. Contact us using
          the details above and we'll link it for you.
        </p>
      </LegalSectionCard>
...
export default ExpertHelpPortal;
