...
const AttorneyPortalCases: React.FC = () => {
  const navigate = useNavigate();
  const { data, isLoading, isError, error } = useAttorneyCases();

  return (
    <AttorneyPortalLayout>
      <Helmet><title>Referring Attorney Portal — Your Cases</title></Helmet>
        ...
        <div className="space-y-2">
          {data.cases.map((c) => (
            <Card
              ...
              onClick={() => navigate(`/external-portal/attorney/cases/${c.appointment_id}`)}
            >
...
export default AttorneyPortalCases;
