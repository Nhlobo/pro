...
async function bridgeToSupabaseAuth(supabaseAdmin: any, account: {
  ...
  // already provisioned on a previous login. Deliberately not
  // supabaseAdmin.auth.admin.listUsers() — called with no pagination
  // arguments (as it was here) it silently returns only the first
  // page of the *entire project's* auth users (50 by default: every
  // attorney, expert, and staff account combined), so past ~50 total
  // users this stops finding people outside that first page and
  // starts wrongly treating their returning login as a first-time
  // one. profiles.id is the same id as auth.users.id and is kept in
  // sync with its email, so a direct lookup here is correct at any
  // scale and is one targeted row fetch instead of a full page scan.
